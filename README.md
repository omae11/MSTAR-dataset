...
| 子集 | 格式 | 大小 | 说明 |
|------|------|------|------|
| [`soc/`](./soc) | 光学 (.jpg) | ~11 MB | SOC 数据集的光学图像版本，便于快速可视化 |
| [`soc_complex/`](./soc_complex) | 复数 (.000 / .025) | ~923 MB | SOC 数据集的原始复数 SAR 数据（实部 + 虚部） |

> 两个子集 **文件名和目录结构完全一致**，只需把 dataloader 指向另一个根目录，就能在光学和复数输入之间切换。
